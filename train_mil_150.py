"""
Attention-MIL training on 150 slides (75 normal + 75 tumor).
Split: 45/15/15 per class = 90 train / 30 val / 30 test.

Key improvement over 50-slide runs:
  30 val + 30 test slides makes threshold calibration reliable.
  90 training slides gives 3x the pattern diversity for the model.

Save to:  /home/subhabrat/sanskar/Aira/camelyon_remaining/train_mil_150.py
Run from: /home/subhabrat/sanskar/Aira/camelyon_remaining/
Command:  CUDA_VISIBLE_DEVICES=0 python3 train_mil_150.py
"""
...
# 150 slides: 75 normal + 75 tumor → 45/15/15 per class
N_TRAIN = 45
N_VAL   = 15
N_TEST  = 15
...
class MILDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feats = torch.load(os.path.join(self.root, f"{name}.pt"),
                           weights_only=True)
...
def collate(batch):
    return [b[0] for b in batch], torch.stack([b[1] for b in batch]), [b[2] for b in batch]
...
def split(labels, seed):
    ...
    rng.shuffle(normal); rng.shuffle(tumor)
    def sp(lst):
...
class GatedAttentionMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.cls(z).squeeze(), a.squeeze()
        ...
        for bag, label, name in zip(fl, lbs, names):
            ...
            if is_attn:
                ...
                if save_attn and attn_dir:
                    ...
                    torch.save(attn.cpu(), os.path.join(attn_dir, f"{name}_attn.pt"))
            else:
                logit = model(bag)
...
def run_seed(seed, labels, device):
    ...
    for name, model, is_attn in [
        ...
        best_auc, best_ckpt = 0.0, f"{RESULTS_ROOT}/seed{seed}_{name}.pt"
        ...
        attn_dir = f"{RESULTS_ROOT}/seed{seed}_{name}_attention"
        test_auc, test_acc, _, _ = evaluate(
            model, mk(te, False), device, is_attn,
            thresh=thresh, save_attn=is_attn, attn_dir=attn_dir)

        results[name] = {"AUC": test_auc, "Acc": test_acc,
                         "thresh": thresh, "best_val_auc": best_auc}
        print(f"  seed={seed} | {name:<22} | "
...
def main():
    ...
    assert len(labels) == 150, f"Expected 150 slides, got {len(labels)}"
    print(f"Split: {N_TRAIN*2} train / {N_VAL*2} val / {N_TEST*2} test")
    print(f"Seeds: {SEEDS}\n")
    ...
    for mname in ["GatedAttentionMIL", "MeanPoolMIL"]:
        best_seed = max(SEEDS, key=lambda s: all_results[s][mname]["best_val_auc"])
        r = all_results[best_seed][mname]
        print(f"  {mname:<25} seed={best_seed} | "
              f"AUC: {r['AUC']:.4f} | Acc: {r['Acc']:.4f}")

    torch.save({"all": all_results}, os.path.join(RESULTS_ROOT, "summary.pt"))
